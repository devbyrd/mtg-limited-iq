...
export const QuickRateModal: React.FC<QuickRateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/70 dark:bg-[#040711]/90 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="relative w-[96vw] max-w-5xl h-[88vh] min-h-[580px] max-h-[820px] bg-white dark:bg-[#090e24] border border-slate-200 dark:border-slate-800/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="shrink-0 flex items-center justify-between gap-3 px-6 py-3.5 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-[#060a1d]">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-auto">
            ...
            {/* Find Similar Cards Top Bar Trigger */}
            <button
              type="button"
              onClick={() => setIsSimilarModalOpen(true)}
              className="px-2.5 py-1 rounded-xl text-xs font-bold bg-violet-50 dark:bg-violet-950/40 text-violet-700 dark:text-cyan-300 hover:bg-violet-100 dark:hover:bg-violet-900/50 border border-violet-200 dark:border-cyan-400/40 flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
              title="Find similar cards from past draft sets to see historical ratings"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-500 shrink-0" />
              <span className="hidden sm:inline">Similar Cards</span>
            </button>
        ...
        <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-5 p-5 sm:p-6 overflow-hidden">
          ...
          <div className="w-full md:w-[320px] lg:w-[360px] shrink-0 flex flex-col items-center justify-start gap-2">
            ...
            <button
              type="button"
              onClick={() => setIsSimilarModalOpen(true)}
              className="w-full mt-1 py-2 px-3 rounded-xl bg-gradient-to-r from-violet-600/10 via-amber-500/10 to-emerald-500/10 hover:from-violet-600/20 hover:to-emerald-500/20 text-slate-800 dark:text-slate-200 border border-violet-300 dark:border-cyan-400/40 text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer group"
              title="Find functionally similar cards in past sets and project a consensus grade"
            >
      ...
      {isSimilarModalOpen && currentCard && (
        <SimilarCardsModal
          ...
          onAdoptGrade={(target, grade) => {
            ...
            const evaluation: UserCardEvaluation = {
              ...
              setCode: target.set,
              userGrade: grade,
              userScore: GRADE_SCORES[grade] || 2.5,
              pickPriority: priority,
              notes: noteText.trim() || undefined,
              updatedAt: new Date().toISOString(),
            };
